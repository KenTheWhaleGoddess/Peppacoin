...
*Setup instructions for a Gitian build of Peppacoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Peppacoin 
...
- [Building Peppacoin Core](#building-peppacoin-core)
...
Clone the git repositories for PeppaCoin and Gitian.
...
in the peppacoin repository under 'Fetch and create inputs' to install sources which require
...
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the peppacoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/peppacoin/.git/
...
For example:
```bash
URL=https://github.com/thrasher-/peppacoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit peppacoin=${COMMIT} --url peppacoin=${URL} ../peppacoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit peppacoin=${COMMIT} --url peppacoin=${URL} ../peppacoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit peppacoin=${COMMIT} --url peppacoin=${URL} ../peppacoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the peppacoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../peppacoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/peppacoin
SIGPATH=/some/root/path/peppacoin-detached-sigs

./bin/gbuild --url peppacoin=${BTCPATH},signature=${SIGPATH} ../peppacoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/peppacoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/peppacoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/peppacoin-osx-build.assert
```
...
[peppacoin-project/gitian.sigs.ltc](https://github.com/garlicoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
